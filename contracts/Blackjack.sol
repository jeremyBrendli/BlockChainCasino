...
//Constructor
Bank public Bankobj;
constructor(address BankAddress) public{Bankobj = Bank (BankAddress); owner = msg.sender;}
...
// Parameters of the game.
  ...
  uint public placedBets;
  uint public number = 2;

//Function to keep track of bet
  function placedBet(uint bet){
    placedBets += bet;
    Bankobj.AdjustChips(bet);
  }
//Function to double the amount of bet (the winnings)
  function setWinnings(bool won){
    if (won){
      winnings = placedBets * number;
...
// Clears the winnings when cashing out
  function clearWinnings(){
    winnings = 0;
  }

/*
        Many of the code below this is implementation meant for transactions between two players(accounts).
        This part was not implemented since we used the bank contract where all the money would go through there.
